"""第5面 AIかみやまコラムのプロンプト定義（Sprint 4 layout swap で旧 page6 から移動）。
...
この記事を読んで、神山さんの視点で 800 字前後の一筆を書いてください。
神山さんが普段読まない領域からの出会いです。
聞き上手の眼差しで、何かが言えれば。

【執筆の注意】
「空・雨・傘」のフレームワーク（事実→解釈→行動）は、神山さんが既に
熟知している思考フレームなので、論評では使わずに、もっと記事固有の
論点に踏み込んでください。各記事ならではの論点・問い・接続を見出す
ことを優先してください。
