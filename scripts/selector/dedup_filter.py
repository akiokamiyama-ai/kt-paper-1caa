...
* **第5面**：N=30 日、セレンディピティ記事1本（page5_url、null 可）
* **第6面**：N=30 日、書籍/音楽/アウトドアの URL（料理は URL なし）
* **第2面 Today's Headlines**：N=7 日（C40, Sprint 8, 2026-05-28）。BBC が
  同じ URL でタイトルだけ更新するパターン（5/27→5/28 cwy22rddy5no 事例）に
  対応するため、headlines にも displayed-URL 記録を導入。
...
Log file shape (``logs/displayed_urls_YYYY-MM-DD.json``):

    {
      ...
      "page5_url": "urlSerendipity",
      "page6_urls": {"books": "urlB", "music": "urlM", "outdoor": "urlO"},
      "headlines_urls": ["urlH1", "urlH2", "urlH3"]
    }
...
``page5_url`` は第5面のセレンディピティ記事 URL（単一）。Sprint 4 layout
swap で旧 page6_url から移動。記事候補ゼロで placeholder 表示の日は ``null``。

``page6_urls`` は第6面の3カラム（books / music / outdoor）のそれぞれの
URL を領域別に持つ dict。Sprint 4 layout swap で旧 page5_urls から移動。
料理カラムは URL なしのため含めない（cooking_history.json で別管理）。

``headlines_urls`` は第2面 Today's Headlines に表示された URL のリスト
（C40, Sprint 8, 2026-05-28 追加）。BBC など同 URL でタイトルだけ更新する
パターンの dedup に使う。5/28 以前のログには存在しないため空リスト扱い。

Public API:

* ``load_displayed_urls_log(target_date)``        → dict | None
* ``write_displayed_urls_log(target_date, page1_urls, page2_urls_by_company,
  page3_urls=None, page4_urls=None, page5_url=None, page6_urls=None,
  headlines_urls=None)``
...
def load_recently_displayed_urls(
    days_back: int,
    page: Literal["page1", "page2", "page3", "page4", "page5", "page6", "headlines"],
    ...
    days_back :
        ...
        uses 7, Page IV uses 30, Page V uses 30, Page VI uses 30.
    page :
        ...
        (3 本 × ローテーション期間)。
        ``"page5"`` collects the single URL from the day's ``page5_url``
        (serendipity 記事 1 本、Sprint 4 layout swap で旧 page6 から移動)。
        ``"page6"`` collects every URL from the day's ``page6_urls`` dict
        across all areas (books / music / outdoor). Cooking has no URL.
        ``"headlines"`` collects every URL from the day's ``headlines_urls``
        list (第2面 Today's Headlines, C40 Sprint 8 で追加)。
    company_key :
        For ``page="page2"``, one of ``"cocolomi"`` / ``"human_energy"`` /
        ``"web_repo"``. Ignored for other pages.
    until_date :
    ...
    for i in range(1, days_back + 1):
        ...
        if page == "page1":
            ...
            for u in log.get("headlines_urls", []) or []:
                if u:
                    urls.add(u)
        else:
...
def write_displayed_urls_log(
    ...
    page5_url: str | None = None,
    page6_urls: dict[str, str | None] | None = None,
    headlines_urls: list[str] | None = None,
) -> Path:
    ...
    ``page5_url`` はセレンディピティ記事 URL の単一文字列または ``None``
    （Sprint 4 layout swap で旧 page6_url から移動）。

    ``page6_urls`` は領域別 dict ``{"books": url|None, "music": url|None,
    "outdoor": url|None}``。料理カラムは URL なしのため含めない。
    ``None`` を渡すと空 dict で書き込み（Sprint 4 layout swap で旧
    page5_urls から移動）。

    ``headlines_urls`` は第2面 Today's Headlines に表示された URL のリスト
    （C40, Sprint 8, 2026-05-28 追加）。``None`` を渡すと空リストで書き込み。
    BBC 等の URL 不変・タイトル更新パターンの dedup に使う。
    """
    ...
    # headlines_urls も順序保持で dedup（top_n=3 なので通常重複しないが防御的に）。
    headlines_seen: set[str] = set()
    unique_headlines: list[str] = []
    for u in headlines_urls or []:
        if u and u not in headlines_seen:
            headlines_seen.add(u)
            unique_headlines.append(u)
    data = {
        ...
        "page5_url": page5_url if page5_url else None,
        "page6_urls": (
            {k: (v if v else None) for k, v in page6_urls.items()}
            if page6_urls is not None else {}
        ),
        "headlines_urls": unique_headlines,
    }
    path = _log_path(target_date)
