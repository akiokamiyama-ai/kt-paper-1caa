    ...
    for art in articles:
        ...
        if penalty <= -10:
            ...
            if pod_excluded:
                ...
                # C26 (Sprint 8, 2026-05-25): Bloomberg は混合フィードを
                # 使うため URL パスで market 以外（Opinion / 動画 /
                # newsletter）を除外する。bloomberg.com 以外には no-op。
                bb_excluded, bb_reason = hard_filter.evaluate_bloomberg_non_market(
                    d.get("url"),
                )
                if bb_excluded:
                    excluded = True
