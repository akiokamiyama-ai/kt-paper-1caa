...
- **fetch_status**: BLOCKED_RUNNER_IP（2026-08-14 確認 / C163）
  - GHA runner から `HTTP 403 Forbidden`。**ローカル WSL からは default UA / browser UA とも 200 OK**（server: AmazonS3）
  - つまりフィードは生きており、**GHA の IP レンジが弾かれている**。UA 変更では回避不可（C130 の JFTC と同型）
  - 採用実績: 2026-04-29 / 04-30 / 05-02 の **3 日のみ**。5/02 を最後に一度も取得できていない
  - 判断: **日次 fetch から除外**（週次プローブは継続）。定義は残す — 実行環境が変われば復活しうる
...
#### 3. 公正取引委員会 報道発表 ❌
- **fetch_status**: BLOCKED_RUNNER_IP（2026-08-14 確認 / C163、経緯は C130）
  - GHA runner から `HTTP 403 Forbidden`（server: **AkamaiGHost**）。**ローカルからは 200 OK**
  - **C130 (Sprint 11) で対策済だが再発**: JftcDriver に実ブラウザ寄りの UA
    (`JFTC_UA`, Firefox 121) を個別 override して一時的に回避できた。しかし
    C130 の調査結論そのものが「**UA 起因ではなく GHA の IP + UA の合わせ技**」であり、
    UA だけでは恒久的な解にならないことは当時から判明していた
  - 実際、C130 修正後に採用できたのは 2026-06-13 / 06-19 / 06-25 / 06-29 / **07-03 の 5 日のみ**で、
    7/03 以降は再び 403。Akamai edge 側のルール更新で UA override が無効化されたと見られる
  - `robots.txt` は `User-agent: * Allow: /` で許可的。**アクセスポリシー違反ではない**（C130 で確認済）
  - 判断: **日次 fetch から除外**（週次プローブは継続）。UA の再変更は同じ轍なので**推奨しない**。
    恒久対策は実行環境を GHA 以外に移すか、取得を諦めるかの二択
  - 代替: フランチャイズ規制リスクの捕捉は Web-Repo 系の他ソース（ビジネスチャンス、
    フランチャイズ関連メディア）が部分的に担う。ただし**一次情報の速報性は失われる**
...
#### 7. リユース経済新聞（リサイクル通信） ✅
- **URL**: https://www.recycle-tsushin.com/
- **RSS**: https://www.recycle-tsushin.com/rss（RSS 2.0、C121 検証時に 200 OK、application/rss+xml、最新記事日次配信）
- **形式**: RSS 2.0
- **mainstream**: false
- **対象**: 中古・リユース・買取業界の総合ニュース（リサイクルショップ、業務用機器、ブランド品、古物商、市場データ、業界リーダー社長インタビュー）
- **位置付け**: C121 (Sprint 11, 2026-07-04) 追加。Web-Repo が扱う **買取・リユース系事業領域** の業界専門誌。ビジネスチャンス偏重の解消 + Web-Repo 事業ドメインとの直接整合。リユース市場は 2 兆円超（2021 年）で成長中、経営判断材料としての価値高い

#### 8. 食品新聞 WEB版（食品新聞社） ✅
- **URL**: https://shokuhin.net/
- **RSS**: https://shokuhin.net/feed/（WordPress 標準 RSS 2.0、C121 検証時に 200 OK、application/rss+xml、hourly 更新）
- **形式**: RSS 2.0
- **mainstream**: false
