...
- **位置付け**: **日本語の国際情勢分析の最重要ソース**だったが、**2026-05-18 に新潮QUE へ統合**され、旧 URL での新規配信が止まっている（旧 RSS は生存も最新 pubDate 2026-05-17 で停滞、在庫枯渇予測）。後継は下記「1b. Shincho QUE」を参照。**C133 (Sprint 12, 2026-07-09) 更新**: 休刊確定として明記。旧 RSS は残置（過去記事に稀に新展開があった場合の保険）だが、C133 で LAYER_3_SOURCES → layer 2 (Haiku prefilter) に降格済。fetch 自体の停止は別途判断（新記事が出ないなら実害なし、rate limit / robots.txt 変更等でエラーが増えたら停止検討）
